...
def pretrained_vq_dir() -> Path:
    configured = os.environ.get("INTENTTALK_PRETRAINED_VQ_DIR")
    if configured:
...
def configure_runtime_env():
    default_cache_root = Path(os.environ.get("INTENTTALK_CACHE_ROOT", repo_path(".cache")))
    default_tmp_root = Path(os.environ.get("INTENTTALK_TMPDIR", repo_path(".tmp")))
